...
Authors: David Mutchler, Valerie Galluzzi, Mark Hays, Amanda Stouder,
         their colleagues and Jonathan Moyers.
...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        x = 0
        temp = ''
        while len(self.contents) < self.volume and len(additional_contents) > x:
            self.contents = self.contents + additional_contents[x]
            x = x+1
        for k in range(x, len(additional_contents)):
            temp = temp + additional_contents[k]
        return temp
    ...
    def shrink(self, new_volume):
        ...
        temp = Box('', new_volume)
        s = temp.append_string(self.contents)
    ...
    def reset(self):
        ...
        self.contents = self.ogc
        self.volume = self.ogv
